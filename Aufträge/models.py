...
class Rechnungsempfänger(models.Model):
    id = models.AutoField(primary_key=True,unique=True,blank=False)
    firma = models.CharField(max_length=120, blank=False)
    adresse = models.CharField(max_length=200, blank=False)
    plz = models.CharField(max_length=20, blank=False)
...
class Auftragspositionen(models.Model):
    ...
    referenz = models.TextField(blank=True,null=True, default='---')
    ...
    unterschrift_datum = models.DateField(blank=True, null=True, default=datetime.date.today())
...
class Rechnung(models.Model):
    ...
    empfänger = models.ForeignKey(Rechnungsempfänger,on_delete=models.PROTECT, blank=False)
