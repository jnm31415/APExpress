  ...
  if request.user.is_authenticated:
    ...
    if request.method == "POST":
      ...
      if user is not None:
        ...
        render(request, 'Aufträge/login.html')
    context={}
    return render(request, 'Aufträge/login.html', context)
  
@login_required(login_url='login')
def main(request):
  akt_auf = Auftrag.objects.filter(auftrag_abgeschlossen = False)
  context = {'akt_auf':akt_auf}
  return render(request, 'Aufträge/main.html', context)
...
def ua10na(request):
  ...
  context={'form':form}
  return render(request, 'Aufträge/UA10NA.html',context)
...
def ua10na_pos(request,pk):
  ...
  return render(request,'Aufträge/UA10NAPos.html',context)
...
def ua12as(request):
  ...
  return render(request, 'Aufträge/UA12AS.html', context)
...
def ua20nr(request):
  ...
  if request.method == "POST":
    ...
    if form.is_valid():
      ...
      return redirect("/")
  context={'form':form}
  return render(request,"Aufträge/UA20NR.html",context)
...
def position(request,pk):
  ...
  return render(request,"Aufträge/Position.html",context)


def help(request):
  hilfe = Help.objects.all()
  context = {'hilfe': hilfe}
  return render(request,'Aufträge/help.html',context)
...
def invoice(request,pk):
  ...
  def generate_pdf(request):
      ...
      im = Image('ap-express.vercel.app/static/media/LogoAP.jpg',4*cm,2*cm,hAlign='RIGHT')
      def myFirstPage(canvas, doc):
        ...
        canvas.drawRightString(19.1*cm,20.2*cm,f"{int(rg.empfänger.id):04d}")
      ...
      data2 = [['Gesamtpreis:',str(total_price)+'€'],['inkl. MwSt:',str(mw)+'€']]
      table2 = Table(data2)
      table2.hAlign = 'RIGHT'
